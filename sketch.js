...
function draw() {
  ...
  if(gameState == 0) {
    ...
    // if(backgroundSprite.x < camera.position.x/1.5) {
      // backgroundSprite.x = camera.position.x;
    // }

    if(backgroundSprite.x < camera.position.x) {
  ...
  camera.position.x = player.x + 320;

  //display score - text
  textSize(28);
  fill(0);
  textFont(marioFont);
  text("Score" + " : " + score, camera.position.x + 210, 65);
}
...
function playerLife() {
  ...
  for(var i = 0; i < hearts; i++) {
    image(heartImage, (60*i)+camera.position.x-370, 40, 55, 55);
  }
